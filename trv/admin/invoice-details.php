...
<body>
	...
	<?php if ($authorizeEnter == true) { ?>
		<div class="contentBox loginBox has-text-centered" style="margin-top: 10px;">
			<?php if ($source == "quickview") { ?>
				<a class="button is-small is-pulled-right backgroundNormal" style="border: none;" href="/trv/admin/invoice-details.php?id=<?php echo $_GET["id"]; ?>&source=web" target="_blank" title="Abrir en una nueva pestaña"><span class="icon is-small"><i class="fas fa-up-right-from-square"></i></span></a>
			<?php } else { ?>
				<a class="button is-small is-pulled-left" style="border: none;" href="/trv/admin/invoices.php"><span class="icon is-small" title="Volver"><i class="fas fa-chevron-left"></i></span></a>
			<?php } ?>

			<div class="box">
				<h3 class="is-size-5">Comprobante de venta <?php echo $saleNumber; ?></h3>
				<p><span class="tag is-rounded"><b>Vendedor: <?php echo $seller; ?></b></span> <span class="tag is-rounded"><?php echo $numberProds; ?> productos</span> <?php if ($cancelled == 1) { ?> <span class="tag is-rounded is-danger is-light">Venta cancelada</span> <?php } ?></p>
				...
				<div class="has-text-left">
					<?php if ($cancelled != 1) { ?>
						...
						<div class="buttons is-centered">
							...
							<button class="button is-info is-light is-fullwidth" onclick="generatePDF(<?php echo $idInvoice; ?>, '<?php echo $saleNumber; ?>')"><i class="fas fa-file-pdf"></i> Generar PDF</button>
						</div>
					<?php } else { ?>
